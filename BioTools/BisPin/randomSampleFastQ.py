#!/usr/bin/python
import SeqIterator, optparse, sys, random
...
def main():
    ...
    p.add_option("--length", "-l", default="-1", help="Length of each sequence to sample.  If the value is negative, then the program will sample from any record.")
...
def start(in_f, out_f, t, length, amount):
    ...
    sys.stdout.flush()
    (seq_lst, seq_count, nt_count) = getSeqCount(in_f, t, length)
    print seq_lst
...
#     if debug():
#         print rands
    getRandomSequences(rands, in_f, out_f, t, length, amount, seq_lst, seq_count)
...
def getRandomSequences(rands, in_f, out_f, t, length, amount, seq_lst, seq_count):
    my_nt_count = 0
    my_rand_count = 0
