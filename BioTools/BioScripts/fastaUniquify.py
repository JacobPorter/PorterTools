...
def fastaUnique(fasta_dir):
    ...
    for f in onlyfiles:
        ...
        sys.stdout.flush()
        temp_fd = open(join(fasta_dir, "temp.fa"), 'w')
        seq_w = SeqGenerator.SeqWriter(temp_fd)
        seq_r = SeqGenerator.SeqGenerator(join(fasta_dir, f))
        last_id = ""
        count = 1
        for s in seq_r:
            parts = s[0].split('#')
        ...
        rename(join(fasta_dir, "temp.fa"), join(fasta_dir, f))
