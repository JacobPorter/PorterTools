...
def main():
    ...
    findEntropy(options.file, options.type, options.verbose)

def findFreq(seq, lookup, offset):
    local_freq = [[0] * 4, [0]*4, [0]*4, [0]*4]
    if offset >= 1:
        if len(seq) > offset:
            seq = seq[offset:]
        else:
            return local_freq
    seq_dimer_len = 0
    for n1 in lookup.keys():
        for n2 in lookup.keys():
            dimer = n1 + n2
            dimer_cnt = seq.count(dimer)
            local_freq[lookup[n1]][lookup[n2]] += dimer_cnt
            seq_dimer_len += dimer_cnt
    for n1 in lookup.keys():
        for n2 in lookup.keys():
            dimer = n1 + n2
...
def findEntropy(filein, t, ignore_ambig, verbose):
    ...
    for rec in SeqIO.parse(filein, t):
        total_seq += 1
        my_seq = str(rec.seq).upper()
        if 'N' in my_seq or 'n' in my_seq:
            continue
